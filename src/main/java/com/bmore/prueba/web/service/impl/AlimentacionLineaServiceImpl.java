...
import com.bmore.prueba.web.repository.old.AlimentacionLineaRepositoryOld;
import com.bmore.prueba.web.service.AlimentacionLineaService;
import com.bmore.prueba.web.utils.Utils;

@Service
public class AlimentacionLineaServiceImpl implements AlimentacionLineaService {

	private static final Logger LOCATION = LoggerFactory.getLogger(AlimentacionLineaServiceImpl.class);
	...
	public OrdenProduccionDTO validaOrden(OrdenProduccionDTO orden) {
		...
		AlimentacionLineaRepositoryOld alimentacionLineaRepositoryOld = new AlimentacionLineaRepositoryOld();
		...
		ordenReturn = alimentacionLineaRepositoryOld.getOrden(orden);

		LOCATION.error("retorno consulta Entrega DAO12345 :" + orden.getOrdenProduccion() + " " + orden.getWerks() + " "
		...
		case 1:// existe orden
			...
			entregaReturn2 = alimentacionLineaRepositoryOld.detalleOrdenProduccion(orden.getOrdenProduccion(),
					orden.getWerks());
			...
			if (entregaReturn2.getContabilizar().equals("false")) {
				...
				if (entregaReturn2.getResultDT().getId() == 1) {
					...
					HashMap<String, String> carrilesBloqueados = alimentacionLineaRepositoryOld.getCarrilesBloqueados("2",
							orden.getWerks());

					for (int x = 0; x < entregaReturn2.getItems().getItem().size(); x++) {

						// Obtiene carriles con materiales bloqueados
						HashMap<String, String> carrilesMaterialBloqueado = alimentacionLineaRepositoryOld
								.getCarrilesMaterialBloqueado(entregaReturn2.getItems().getItem().get(x).getMaterial(),
										orden.getWerks());

						CarrilesUbicacionDTO carrilesDAO = alimentacionLineaRepositoryOld.getCarriles(orden.getWerks(),
		...
		LOCATION.error("retorno consulta OrdenProduccion:" + orden.getOrdenProduccion() + " " + orden.getWerks() + " "
				+ ordenReturn.getResultDT().getId() + " " + ordenReturn.getResultDT().getMsg()
				+ ordenReturn.getFabrica() + " " + ordenReturn.getFechaDocumento());
		return ordenReturn;
	}
	...
	public ResultDTO ingresaDetalleEnvaseBO(CarrilesUbicacionDTO carriles, String user, String werks) {
		...
		return alimentacionLineaRepositoryOld.ingresaDetalleEnvase(carriles.getItem().get(0).getEntrega(), carriles, user,
				werks);
	}
	...
	public int limpiarPendientes(String vbeln) {
		...
		return alimentacionLineaRepositoryOld.limpiaPendientes(vbeln);
	}
