...
public class RecepcionEnvaseRepository {
  ...
  public EntregaDTO getEntrega(EntregaDTO entregaInput) {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        log.error("ENTREGA TIPO:  " + entregaInput.toString());
  ...
  public HashMap<String, String> getLgortsEntrega(String entrega) {
    ...
    try {
      jdbcTemplate.query(ENTRY_LGORTS, new RowMapper<String>() {
      ...
      map.put("resultDT.msg", "Error en getLgortsEntrega: Exception -> " + e.toString());

      result.setId(2);
      result.setMsg(e.getMessage());

      log.error("Error en getLgortsEntrega: Exception -> ", e);
    }
        ...
        if (rs.getInt(1) >= 1) {
          ...
          while (rs2.next()) {
            ...
            if (map.get(rs2.getString("MATNR")) == null) {
              ...
              log.error("MATNR: " + rs2.getString("MATNR"));
              ...
              if (entrega.getLfart().equals("EL") || entrega.getLfart().equals("YD15")
                ...
                if (entrega.getWerksBCPS() != null && (entrega.getWerksBCPS().equals("EMZ1"))
                  ...
                  log.error("envases 1.5 HUS");

                } else {
                  log.error("VIDRIO 1.5 HUS");
              ...
              if (rs3.next()) {
                ...
                log.error("cantidad: " + item.getHus());
              ...
              log.error("Buscando HUs asignadas");
              ...
              try {
                ...
                log.error("Error en SUMA_ZPICKING_ENTREGA_ENTRANTE_PENDIENTES: " + e);
              }
              ...
              try {
                ...
                log.error("Error en SUMA_ZPICKING_ENTREGA_ENTRANTE_TOTAL: " + e);
              }
  ...
  public CarrilesUbicacionDTO getCarriles(String WERKS, String MATNR, String ID_PR, String ID_PR_Z, String LGORT,
    ...
    try {
      ...
      for (int x = 0; x < carrilList.size(); x++) {
        ...
        if (Integer.parseInt(item.getCantidadHus().trim()) != 0) {
          ...
          try {
            ...
            log.error("Error: PENDIENTES_POR_PICKEAR_POR_CARRIL " + e.toString());
          }
      ...
      log.error("Error SQL:" + en.toString());
    } finally {
      try {
        // DBConnection.closeConnection(con);
      } catch (Exception e) {
  ...
  public ResultDTO aumentaInventario(EntregaInputDTO entrega, String hu) {
    ...
    try {
      ...
      log.error("Se consume inventario: ");
      ...
      if (stmn.executeUpdate() > 0) {
        ...
        log.error("recepcion LGNUM: " + entrega.getuDestino0() + " LGTYP: " + entrega.getuDestino1()
  ...
  public ResultDTO insertProcesoContingencia_4_14_32(String werks, String VBELN, String lfart, String user) {
    ...
    try {
      log.error("antes de ejecutar insert 4");
      ...
      log.error("WERKs: " + werks);
      log.error("vbeln: " + VBELN);
      ...
      log.error("SQL Exception 4to paso: " + e.toString());
      result.setId(2);
      result.setMsg(e.getMessage());
    } catch (Exception en) {
      log.error("SQL Exception 4to paso: " + en.toString());
      result.setId(2);
      result.setMsg(en.getMessage());
    } finally {
      try {
  ...
  public EntregaInputDTO validarEntregaPickinCompleto(EntregaInputDTO entregaInput) {
    ...
    try {
      ...
      log.error("Error validaEntregaPickingCompleto");
      stmn = con.prepareStatement(VALIDA_PICK_FINALIZADO);

      stmn.setString(1, Utils.zeroFill(entregaInput.getEntrega(), 10));
      stmn.setString(2, entregaInput.getWerks());

      log.error("Error: " + Utils.zeroFill(entregaInput.getEntrega(), 10));

      log.error("Error: " + entregaInput.getWerks());
      rs = stmn.executeQuery();

      log.error("Error despues");
      if (rs.next()) {
