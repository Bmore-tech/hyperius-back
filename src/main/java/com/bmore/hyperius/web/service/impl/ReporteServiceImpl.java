...
public class ReporteServiceImpl implements ReporteService {
  ...
  @Autowired
  private ServletReporteRepository servletReporteRepository;
  ...
  public File getDocument(String reporte, String myTimeStamp, String werks, String vbeln) {
    ...
    if (reporte.equals("PROFORMA")) {
      ...
      file = generateShipping(reporte, filePath, servletReporteRepository.getDatosShipping(werks));
    } else if (reporte.equals("REPORTE3")) {

    } else if (reporte.equals("EDI")) {
      String tknum = servletReporteRepository.getTknum(vbeln);
      ...
      file = generateEntregasCSC(reporte, filePath, creacionEntregasRepository.getEntregas());
    } else if (reporte.equals("RESUMEN_OPERACIONES")) {
      file = generateReporteOperaciones(filePath, servletReporteRepository.getReporteOperacionesDAO(werks),
          servletReporteRepository.getReporteOperacionesInitialStockDAO(werks));
    }
  ...
  public File generateEdi(String EDI, String filepath) {
    ...
    try {
      ...
      log.error("ServletReporteBO generateShipping fileNotFound:" + e.getMessage());
    } catch (UnsupportedEncodingException e) {
      log.error("ServletReporteBO generateShipping unsupported:" + e.getMessage());
    }
    return file;
  }
  ...
  public File generateEntregasCSC(String type, String filepath, CreacionEntregasDTO creacionEntregasDTO) {
    ...
    try {
      ...
      log.error("ServletReporteBO generateEntregasCSC fileNotFound:" + e.getMessage());
    } catch (UnsupportedEncodingException e) {
      log.error("ServletReporteBO generateEntregasCSC unsupported:" + e.getMessage());
    }
    return file;
  }
